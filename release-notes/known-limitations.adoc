...
Se vuoi gestire un cluster su un'altra istanza di Astra Control Center, dovresti prima link:../use/unmanage.html#unmanage-a-cluster["annullare la gestione del cluster"] dall'istanza su cui è gestito, prima di gestirlo su un'altra istanza. Dopo aver rimosso il cluster dalla gestione, verifica che il cluster non sia più gestito eseguendo questo comando:
...
. https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy.html["Installare la versione più recente di Astra Trident"^].
...
Assicurati di link:../use/monitor-protect.html#add-a-proxy-server-for-connections-to-the-netapp-support-site["inserire i valori corretti"] quando stabilisci una connessione.
