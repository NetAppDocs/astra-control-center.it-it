...
+
** Il token del file di configurazione predefinito di VMware TKG e TKGi scade dieci ore dopo la distribuzione. Se usi i prodotti del portfolio Tanzu, devi generare un file di configurazione del cluster Kubernetes Tanzu con un token senza scadenza per evitare problemi di connessione tra Astra Control Center e i cluster di applicazioni gestiti. Per istruzioni, visita  https://techdocs.broadcom.com/us/en/vmware-cis/nsx/vmware-nsx/4-2.html["la documentazione del prodotto VMware NSX-T Data Center"^].
