...
NOTE: Quando aggiungi un bucket, seleziona il provider di bucket corretto e fornisci le credenziali giuste per quel provider. Ad esempio, l'interfaccia utente accetta NetApp ONTAP S3 come tipo e accetta le credenziali StorageGRID; tuttavia, questo farà sì che tutti i futuri backup e ripristini delle app che usano questo bucket falliscano. Vedi la link:../release-notes/known-issues.html["Note di rilascio"].
