...
IMPORTANT: Se l'applicazione utilizza una classe di storage supportata da `ontap-nas-economy` driver, non è possibile utilizzare policy di protezione. Utilizzare una classe di storage alternativa se si desidera un criterio di protezione.
...
IMPORTANT: Se l'applicazione utilizza una classe di storage supportata da `ontap-nas-economy` driver, le snapshot non possono essere utilizzate. Utilizzare una classe di storage alternativa per gli snapshot.
