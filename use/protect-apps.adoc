...
=== [Tech preview] Crea un backup pianificato degli spazi dei nomi

È possibile creare un backup pianificato e automatico di uno o più namespace in un cluster. Questi flussi di lavoro vengono forniti da NetApp come file CR associati a uno script Python. Quando si configurano e installano i flussi di lavoro, i backup dello spazio dei nomi vengono creati automaticamente in base a una pianificazione scelta durante l'installazione. Puoi proteggere tutti i namespace su un cluster con il workflow completo del backup del cluster o solo alcuni namespace con il workflow di backup basato su etichetta.

Quando al cluster viene aggiunto un nuovo namespace o un'applicazione che rientra nell'ambito della pianificazione del backup, queste vengono automaticamente protette.

.Esempio: Backup completo del cluster
Ad esempio, quando si configura e installa l'intero flusso di lavoro del backup del cluster, viene eseguito periodicamente il backup di tutte le applicazioni in qualsiasi namespace, senza ulteriori interventi da parte dell'amministratore. Lo spazio dei nomi non deve esistere al momento dell'installazione del flusso di lavoro; se in futuro viene aggiunto uno spazio dei nomi, verrà protetto.

.Esempio: Backup basato su etichette
